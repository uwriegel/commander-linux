...
impl App {
    pub fn new() -> Self {
        ...
        let app = Self { application: application.clone() };
        let app_clone = app.clone();
        application.connect_activate(move|_| { app_clone.build_ui()});
        app
    }
    ...
    fn build_ui(&self) {
        ...
        self.append_column(&left_commander, 0);
        self.append_column(&left_commander, 1);
        self.append_column(&left_commander, 2);
        let model = self.create_and_fill_model();
        left_commander.set_model(Some(&model));
    
        self.append_column(&right_commander, 0);
        self.append_column(&right_commander, 1);
        self.append_column(&right_commander, 2);
        let model = self.create_and_fill_model();
        ...
        self.application.set_accels_for_action("app.viewer", &["F3"]);
        ...
        self.application.add_action(&action);
        self.application.set_accels_for_action("app.showhidden", &["<Ctrl>H"]);
    ...
    fn append_column(&self, tree: &TreeView, id: i32) {
    ...
    fn create_and_fill_model(&self) -> ListStore {
